...
TEST(BackupTest, backup) {
    ...
    fs::path prevBackupDir{};
    do {
        Backup bkp{source, target, date};
        ASSERT_EQ(target/ Backup::META_DIR/ "1976"/ "07"/ "15_000", bkp.backup_dir());
        bkp.run();
        ...
        ASSERT_FALSE(exists(target/Backup::PREVIOUS_LINK));
        ASSERT_TRUE(exists(target/Backup::CURRENT_LINK));
        ASSERT_EQ(canonical(bkp.backup_dir()), canonical(target/Backup::CURRENT_LINK));
        prevBackupDir = bkp.backup_dir();
    } while (false);

    do {
        Backup bkp1{source, target, date};
        ASSERT_EQ(target/ Backup::META_DIR / "1976"/ "07"/ "15_001", bkp1.backup_dir());
        ...
        ASSERT_EQ(5, std::filesystem::hard_link_count(backup1File));
        ASSERT_EQ(5, std::filesystem::hard_link_count(backupFile));
        ASSERT_EQ(5, std::filesystem::hard_link_count(backupDigest));
        ...
        ASSERT_TRUE(exists(target/Backup::PREVIOUS_LINK));
        ASSERT_EQ(canonical(prevBackupDir), canonical(target/Backup::PREVIOUS_LINK));
        ASSERT_TRUE(exists(target/Backup::CURRENT_LINK));
        ASSERT_EQ(canonical(bkp1.backup_dir()), canonical(target/Backup::CURRENT_LINK));
        prevBackupDir = bkp1.backup_dir();
    } while (false);
    do {
        // One more time (o test case where ./previous exists
        Backup bkp2{source, target, date};
        bkp2.run();

        ASSERT_TRUE(exists(target/Backup::PREVIOUS_LINK));
        ASSERT_EQ(canonical(prevBackupDir), canonical(target/Backup::PREVIOUS_LINK));
        ASSERT_TRUE(exists(target/Backup::CURRENT_LINK));
        ASSERT_EQ(canonical(bkp2.backup_dir()), canonical(target/Backup::CURRENT_LINK));
    } while (false);
}
